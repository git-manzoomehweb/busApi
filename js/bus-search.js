...
function renderSeatAvailableCount(count, ver) {
  const c = Number(count);
  if (!Number.isFinite(c) || c <= 0) return "";

  if (ver === "mobile") {
    return (
      `<p class='book-text-zinc-900 book-text-sm book-font-bold'>${c}</p>` +
      `<p class='book-text-xs book-text-zinc-400'>صندلی خالی</p>`
    );
  }
  return `<p class="book-text-zinc-800">${c} صندلی خالی</p>`;
}
