/**
 * Global variables to store flight search and booking data from localStorage.
 * Initialized as null to be populated on DOM load.
 */
let selectedMode = null;
let flightSearchLocalStorage = null;
let flightReserveLocalStorage = null;
let ExcessService = null;
let SeatSelection = null;
let dictionaries = [];
let schemaId; // Flight schema ID (e.g., 291: one-way, 290: round-trip, 292: multi-city)
...
/**
 * Event listener for DOM content loaded to initialize the flight booking process.
...
document.addEventListener("DOMContentLoaded", function () {
    try {
        ...
        if (localStorage.getItem('flightSearch')) {
            selectedMode = 'flight';
        } else if (localStorage.getItem('busSearch')) {
            selectedMode = 'bus';
        }
        ...
        if (utmSource === "safarmarket") {
            ...
            if (container) {
                const bannerHTML = `
                <div class="book-bg-orange-500 book-text-white book-text-sm book-rounded-2xl book-p-4 book-text-center book-font-bold book-mb-4">
            ...
            document.cookie = `safarmarketId=sm-1mzt2xnkgfz; path=/; expires=${expiryDate.toUTCString()}; Secure; SameSite=Lax`;

            // Clear localStorage keys that are no longer needed
            localStorage.removeItem("flightSearch");
            localStorage.removeItem("flightAmenities");

        } else if (localStorage.getItem("flightSearch")) {
            // Parse stored flight search data
            flightSearchLocalStorage = JSON.parse(localStorage.getItem("flightSearch"));

            // If booking type is AI, set AI source and update research button
            if (flightSearchLocalStorage?.Mode === "AI") {
                $bc.setSource("cms.flightAi", [{
                    TokenId: flightSearchLocalStorage.TokenId,
                    FlightId: flightSearchLocalStorage.FlightId,
                    run: true
                }]);
                ...
                flightReserveLocalStorage = localStorage.getItem("flightBook")
                    ? JSON.parse(localStorage.getItem("flightBook"))
                    : "";
                setFlightGroup();
            }
        }
...
const renderRoutesInfo = async (element) => {
    try {
         ...
         * @param {string} icon - The SVG icon identifier.
         * @param {string} label - The label for the information.
         * @param {string} value - The value to display.
         * @returns {string} HTML string for airline info or empty string.
         */
         ...
         * @param {Object} item - Route data.
         * @param {Object} baggage - Baggage information.
         * @param {number} index - Route index.
         * @returns {string} HTML string for the route.
         */
                    ...
                    <div class="book-flex">
                        ...
                        <div class="book-flex book-flex-col book-border-l book-border-zinc-300 book-px-2 book-ml-3">
                            ...
                            <div class="book-text-sm book-text-primary-400 book-my-10 book-w-40">
                                <span class="book-ml-1">${await renderFormatterDuration(item.Duration)}</span>مدت پرواز
                            </div>
                            <div>
                        ...
                        <div class="book-flex book-flex-col">
                            ...
                            <div class="book-text-sm book-my-5">
                                <div class="book-grid book-grid-cols-2 book-gap-2">
                                    ${renderAirlineInfo("class-details-icon", "کلاس بلیط", `${await renderFlightClass(item.Class)}`)}
...
const renderAirlineCode = async (element) => {
    try {
        ...
        console.error(`renderAirlineCode: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
        return "";
    }
};
...
const renderAirport = async (element) => {
    try {
        ...
        console.error(`renderAirport: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
        return "";
    }
};
...
const renderCurrency = async (element, type) => {
    try {
        ...
        console.error(`renderCurrency: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
        return "";
    }
};
...
const renderListApi = async () => {
    try {
        ...
        console.error(`renderListApi: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const renderInfoApi = async () => {
    try {
        ...
        console.error(`renderInfoApi: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const renderCheckCoupon = async (element) => {
    try {
        ...
        $bc.setSource("cms.checkCoupon", [{
            ...
            Group: JSON.stringify(flightReserveLocalStorage.FlightGroup),
            Id: flightReserveLocalStorage.FlightId,
...
const renderCounter = async () => {
    try {
        ...
        console.error(`renderCounter: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const renderCompanyRule = async () => {
    try {
        ...
        console.error(`renderCompanyRule: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const renderBankList = async (element) => {
    try {
        ...
        console.error(`renderBankList: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
/**
...
const renderResearch = async () => {
    try {
        ...
        if (!flightSearchLocalStorage || !flightSearchLocalStorage.Schemaid) {
            throw new Error("Missing Schemaid in flightSearchLocalStorage");
        }
        if (!flightReserveLocalStorage || !flightReserveLocalStorage.FlightGroup || !flightReserveLocalStorage.PriceInfo) {
            throw new Error("Invalid or missing data in flightReserveLocalStorage");
        }
        ...
        const flightClass = flightReserveLocalStorage.FlightGroup[0].Class;
        ...
        const passengerFare = flightReserveLocalStorage.PriceInfo.PassengerFare;
        const passengerParts = [];
        if (passengerFare[0].Count > 0) passengerParts.push(`${passengerFare[0].Count} بزرگسال`);
        if (passengerFare[1].Count > 0) passengerParts.push(`${passengerFare[1].Count} کودک`);
        if (passengerFare[2].Count > 0) passengerParts.push(`${passengerFare[2].Count} نوزاد`);
        ...
        // Update trip details
        if (flightReserveLocalStorage.FlightGroup.length === 0) {
        ...
        if (schemaId === 291) {
            ...
            departureDate.value = convertToPersianDate(flightGroup[0].DepartureDate) || "";
            departureDate.dataset.date = flightGroup[0].DepartureDate || "";
            arrivalDate.value = convertToPersianDate(flightGroup[lastIndex].ArrivalDate) || "";
        ...
        console.error(`renderResearch: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
/**
...
const toggleServiceTable = (element, serviceItemId) => {
    try {
        ...
        console.error(`toggleServiceTable: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
/**
        ...
        if (parent === 'book-services__container') {
            ...
            if (element.dataset.run === "0" && apiContainer.querySelector(".book-api__container__loader")) {
                ...
                $bc.setSource("cms.rule", {
                    type,
                    SessionId: flightSearchLocalStorage.SessionId,
                    FlightId: flightReserveLocalStorage.FlightId,
                    FlightGroup: JSON.stringify(flightReserveLocalStorage.FlightGroup),
                    rkey: rkey,
                    run: true
                });
                element.setAttribute("data-run", "1");
...
const togglePassengerType = (element, add, remove) => {
    try {
        ...
        console.error(`togglePassengerType: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const toggleVisibility = (showClass, hideClass) => {
    try {
        ...
        console.error(`toggleVisibility: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const toggleDropItem = (element, type, load) => {
    try {
        ...
        console.error(`toggleDropItem: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
/**
...
const toggleSelectItem = (element) => {
    try {
        ...
        console.error(`toggleSelectItem: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
}
...
const onProcessedCheckCoupon = async (args) => {
    try {
            ...
            if (responseJson) {
                ...
                if (Number(responseJson.code) === 1) {
                    ...
                    if (responseJson.coupon_price?.unit === 'percent') {
                        ...
                        updatePrices(responseJson.buy_price?.cost, responseJson.buy_price?.firstpay); // Assumed to be defined elsewhere
                    } else {
        ...
        console.error(`onProcessedCheckCoupon: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};

/**
 * Processes Safarmarket flight API response, updates localStorage, and displays warning modal if needed.
 * @param {Object} args - API response object containing status and data.
 */
const onProcessedFlightSafarmarket = async (args) => {
    try {
        ...
        localStorage.setItem("flightBook", JSON.stringify(responseJson));

        // Update global variable
        flightReserveLocalStorage = responseJson;

        // Initialize or update flightSearch in localStorage
        localStorage.setItem("flightSearch", JSON.stringify({}));
        flightSearchLocalStorage = localStorage.getItem("flightSearch")
        ...
        if (Array.isArray(responseJson.FlightGroup)) {
            ...
            if (flightGroupCount === 1) {
                flightSearchLocalStorage.Schemaid = 291;
                flightSearchLocalStorage.Type = "Flight";
            } else if (flightGroupCount === 2) {
                flightSearchLocalStorage.Schemaid = 290;
                flightSearchLocalStorage.Type = "Flight";
            }
            // Update flightSearch in localStorage
        ...
        console.error(`onProcessedFlightSafarmarket: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};

/**
 * Processes flight group API response, updates localStorage, and displays a warning modal if needed.
...
const onProcessedFlightAi = async (args) => {
    try {
        ...
        flightSearchLocalStorage.SessionId = responseJson.SessionId;
        localStorage.setItem("flightSearch", JSON.stringify(flightSearchLocalStorage));
        localStorage.setItem("flightBook", JSON.stringify(responseJson));

        // Update flightReserveLocalStorage
        flightReserveLocalStorage = localStorage.getItem("flightBook")
            ? JSON.parse(localStorage.getItem("flightBook"))
            : "";
        if (!flightReserveLocalStorage) {
            throw new Error("Failed to parse flightBook from localStorage");
        }
        ...
        console.error(`onProcessedFlightAi: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const onProcessedFlightApiRule = async (args) => {
    try {
        ...
        const generatePassengerTable = (tableId, type) => {
            ...
            const passengers = flightReserveLocalStorage?.PriceInfo?.PassengerFare || [];
        ...
        // Render flight services (baggage, meals)
            ...
            const routesInfo = flightReserveLocalStorage?.FlightGroup?.flatMap(group => group.RoutesInfo) || [];
        ...
        console.error(`onProcessedFlightApiRule: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const onProcessedSupplierCredit = async (args) => {
    try {
        ...
        console.error(`onProcessedSupplierCredit: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const onProcessedPreviousPassengers = async (args) => {
    try {
        ...
        console.error(`onProcessedPreviousPassengers: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const onProcessedUserCredit = async (args) => {
    try {
        ...
        if (response.status === 200) {
            ...
            if (responseJson) {
                ...
                if (userCredit > firstPay) {
                    // Render credit payment option
                    document.querySelector(".book-invoice__container")?.insertAdjacentHTML("beforeend", `
                        <div class="book-invoice__content book-cursor-pointer book-p-2" data-run="0" onclick="submitInvoice(this,'credit__Invoice')">
                            <ul class="book-flex book-justify-between">
                                <li>${document.querySelector(".book-buyers__container").dataset.accounttype === "3" ? "پرداخت از طریق کیف پول" : "پرداخت اعتباری"}</li>
                                <li></li>
                                <li><img src="/booking/images/credit-booking.png" alt="creditlogo" width="50" height="50" /></li>
                                <li><p>اعتبار باقی مانده :</p>
                                    <span class="book-price_remaining_of_credit">${new Intl.NumberFormat().format(userCredit - firstPay)}</span>
                                    <span class="book-text-xs book-mx-1">${document.querySelector(".book-unit__content span").textContent}</span></li>
                                <li><span class="book-totalcom__cost">${document.querySelector(".book-firstpay__cost").textContent}</span>
                                    <span class="book-text-xs book-mx-1">${document.querySelector(".book-unit__content span").textContent}</span></li>
                                <li><button type="button" class="book-btn__content book-text-white book-rounded-2xl book-p-3 book-cursor-pointer book-text-center book-bg-primary-400 book-next__btn hover:book-bg-secondary-400">تایید و پرداخت</button></li>
                            </ul>
                        </div>`);
                }
            }
        ...
        console.error(`onProcessedUserCredit: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const setFlightGroup = async () => {
    try {
        // Load flight booking data from localStorage
        if (localStorage.getItem("flightBook")) {
            const dict = flightReserveLocalStorage.dictionaries;
        ...
        $bc.setSource("flight.book", flightReserveLocalStorage);
        flightSearchLocalStorage = localStorage.getItem("flightSearch")
            ? JSON.parse(localStorage.getItem("flightSearch"))
            : null;

        // Extract passenger counts from search data
        const Adults = flightReserveLocalStorage.PriceInfo.PassengerFare[0].Count;
        const Children = flightReserveLocalStorage.PriceInfo.PassengerFare[1].Count;
        const Infants = flightReserveLocalStorage.PriceInfo.PassengerFare[2].Count;
        ExcessService = flightReserveLocalStorage.FlightGroup?.[0]?.ExcessService || false;
        SeatSelection = flightReserveLocalStorage.FlightGroup?.[0]?.SeatSelection || false;

        // Show UI for extra services (baggage, meals, seat) if enabled
        if (ExcessService || SeatSelection) {
            ...
            div.innerHTML = `
                <div class="book-services__container book-api__container book-bg-white book-shadow-md book-border-slate-600 book-rounded-2xl book-border-b-3 book-p-5 book-mb-10">
                    <div class="book-text-base book-flex book-justify-between book-items-center book-cursor-pointer" onclick="toggleContentApi(this,'','book-services__container')" data-run="0">
                        <div> گزینه‌های اضافی
                            <div class="book-text-zinc-500 book-text-xs book-mt-2 book-mb-5">
                                با رزرو صندلی دلخواه، انتخاب پیشاپیش وعده غذایی مورد علاقه‌تان، یا خرید ظرفیت بار اضافی، با راحتی پرواز کنید.
                            </div>
                        </div>
                    ...
                    <div class="book-api__content book-hidden">
                        <div class="book-tab__navigation__container book-flex book-justify-around book-space-x-2 book-space-x-reverse book-mb-10">
                            <button onclick="selectServiceTab(this,'baggageService','ExcessService')" data-run="0" type="button" class="book-tab__navigation__content book-min-w-52 book-p-4 book-rounded-xl book-border-zinc-400 book-border book-transition-colors book-text-zinc-500 hover:book-bg-primary-400 hover:book-border-primary-400 hover:book-text-white">
                                <svg width="51" height="50"><use href="/booking/images/sprite-booking-icons.svg#wheel-bag-icon"></use></svg>
                                <h6 class="book-text-sm">بار اضافه</h6>
                            </button>
                            <button onclick="selectServiceTab(this,'mealService','ExcessService')" data-run="0" type="button" class="book-tab__navigation__content book-min-w-52 book-p-4 book-rounded-xl book-border-zinc-400 book-border book-transition-colors book-text-zinc-500 hover:book-bg-primary-400 hover:book-border-primary-400 hover:book-text-white">
                                <svg width="41" height="50"><use href="/booking/images/sprite-booking-icons.svg#meal-icon"></use></svg>
                                <h6 class="book-text-sm">وعده غذایی</h6>
                            </button>
                            <button onclick="selectServiceTab(this,'seatService','SeatAvailability')" data-run="0" type="button" class="book-tab__navigation__content book-min-w-52 book-p-4 book-rounded-xl book-border-zinc-400 book-border book-transition-colors book-text-zinc-500 hover:book-bg-primary-400 hover:book-border-primary-400 hover:book-text-white">
                                <svg width="41" height="50"><use href="/booking/images/sprite-booking-icons.svg#seat-icon"></use></svg>
                                <h6 class="book-text-sm">انتخاب صندلی</h6>
                            </button>
                        </div>
        ...
        // Determine if the flight is internal (domestic) from dictionaries
        const internal = dictionaries[0]?.internal || false;
        // Show appropriate passenger container based on flight type (internal/external)
        if (internal) {
            ...
            addPassenger(".book-passengers__container__internal", Adults, Children, Infants); // Assumed to be defined elsewhere
        } else {
            ...
            addPassenger(".book-passengers__container__external", Adults, Children, Infants); // Assumed to be defined elsewhere
        }
        ...
        getWithExpiry("flightSearch", "flightBook", "flightAmenities"); // Assumed to be defined elsewhere
        ...
        originalFirstPay = flightReserveLocalStorage.PriceInfo.TotalCommission;
        originalTotalCom = flightReserveLocalStorage.PriceInfo.TotalCommission;
        originalTotal = flightReserveLocalStorage.PriceInfo.Total;
    } catch (err) {
        console.error(`setFlightGroup: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const setSession = async (args) => {
    try {
        ...
        // Load or initialize flight search data from localStorage
        let currentFlightSearch = localStorage.getItem("flightSearch");
        flightSearchLocalStorage = currentFlightSearch ? JSON.parse(currentFlightSearch) : {};

        // Update session ID and expiry
        flightSearchLocalStorage.SessionId = newSessionId;
        flightSearchLocalStorage.Expiry = now.getTime() + ttl;

        // Save updated flight search data to localStorage
        localStorage.setItem("flightSearch", JSON.stringify(flightSearchLocalStorage));

        // Initialize flight group UI
        await setFlightGroup(); // Assumed to be defined elsewhere
    } catch (err) {
        console.error(`setSession: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const warningConfirm = (element) => {
    try {
        ...
        console.error(`warningConfirm: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const warningReject = (element) => {
    try {
        ...
        console.error(`warningReject: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
    ...
    try {
        ...
        for (let row of rows) {
            ...
            if (secondCellText === 'انتخاب نشده') {
                ...
                row.cells[2].insertAdjacentHTML("beforeend", ` <p class="book-text-red-600 book-mt-1 book-cursor-pointer" onclick="removePassengerServices(this)">حذف</p>`);
                ...
                const allRowsFilled = Array.from(rows).slice(1).every(r => r.cells[1]?.textContent?.trim() !== 'انتخاب نشده');
                if (allRowsFilled) {
        ...
        console.error(`updatePassengerServices: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
...
const removePassengerServices = (element) => {
    try {
        ...
        console.error(`removePassengerServices: ${err.message}, Line: ${err.lineNumber || 'unknown'}`);
    }
};
 ...
 * @param {string} key - Primary key for localStorage (e.g., 'flightSearch').
 * @param {string} key2 - Secondary key for localStorage (e.g., 'flightBook').
...
const addPassenger = (parentSelector, adults, children, infants) => {
    try {
        ...
        const FlightId = flightReserveLocalStorage.FlightId;
    ...
    try {
        ...
        passengerInfoContents.forEach(content => {
            if (!content.parentElement.classList.contains("book-hidden")) {
                ...
                element.className = "book-summary__bodys book-grid book-grid-cols-4 book-gap-1 book-mb-4";
    ...
    try {
        ...
        document.querySelectorAll(".book-passenger__container").forEach(passengerElement => {
            if (!passengerElement.closest(".book-passengers__content").classList.contains("book-hidden")) {
                ...
                if (seatIdAttr) {
                    passengerData.seatId = seatIdAttr.split(",").map(id => id.trim());
                }

                // بررسی و اضافه کردن serviceId در صورت وجود
                const serviceIdAttr = passengerElement.getAttribute("data-serviceId");
                if (serviceIdAttr) {
                    passengerData.serviceId = serviceIdAttr.split(",").map(id => id.trim());
                }
        ...
        const formData = {
            SessionId: flightSearchLocalStorage.SessionId,
            FlightId: flightReserveLocalStorage.FlightId,
            FlightGroup: flightReserveLocalStorage.FlightGroup,
            Schemaid: flightSearchLocalStorage.Schemaid,
...
const nextStep = (element) => {
    ...
    if (step === "passenger") {
        ...
        if (isExist) {
                    ...
                    if (passportCodeField && passportCodeField.classList.contains("book-Required")) {
                        ...
                        if (placeOfBirth && placeOfBirth.value === "1002236") {
                            if (!regex.test(passportCode)) {
                                addError(passportCodeField, "شماره گذرنامه صحیح نیست.");
                                isValid = false;
                            } else {
                        ...
                        if (placeOfBirth && placeOfBirth.value === "1002236") {
                            ...
                            if (checkArray < 10) {
                                ...
                                if ((sum < 2 && check === sum) || (sum >= 2 && check + sum === 11)) {
                                    ...
                                    addError(nationalCodeField, "کد ملی صحیح نیست.");
                                    isValid = false;
                                }
                            } else {
                                addError(nationalCodeField, "کد ملی صحیح نیست.");
                    ...
                    passengerContent.querySelectorAll(".book-EnglishKey").forEach(englishField => {
                        if (englishField.classList.contains("book-Required")) {
                            if (englishField.value.length < 2) {
                                addError(englishField, "حداقل تعداد کاراکتر 2 است.");
                                isValid = false;
                            } else {
                                const regex = /^[a-zA-Z ]+$/;
                                if (!regex.test(englishField.value)) {
                                    addError(englishField, "کاراکتر غیر مجاز وارد شده است.");
                                    isValid = false;
                                } else {
                        ...
                        if (countryField.classList.contains("book-Required")) {
                            if (countryField.getAttribute("data-value") === '') {
                                ...
                                countryField.closest(".book-info__item__container").insertAdjacentHTML('beforeend',
                                    `<div class="book-alert__content book-text-red-600 book-text-xs book-mt-2 book-float-right">کشور را انتخاب کنید.</div>`);
                                isValid = false;
                            } else {
            ...
            if (isValid) {
                ...
                if (mainUserId === "0") {
                    ...
                    document.querySelector(".book-current__route__map").innerText = 'مشخصات خریدار';
        ...
        if (isValid) {
            ...
            const share = flightSearchLocalStorage.share;
            if (Number(share) === 1) {
...
const prevStep = (element) => {
    try {
        ...
        if (step === "buyer") {
            ...
            document.querySelector(".book-current__route__map").innerText = 'مشخصات مسافران';
            element.classList.add("book-hidden");
            updateStep("مشخصات مسافران", element);
        } else if (step === "summary") {
            ...
            updateStep("مشخصات خریدار", element);
        }
    } catch (err) {
