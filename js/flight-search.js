...
const setCalendarLookUp = async (args) => {
    try {
        ...
        const prices = args.source.rows.slice(0, 30)
            .map(row => priceWithCurrency(row.min_price, { as: 'number' }));
            ...
            if (!content.querySelector(".book-time__content")) {
                ...
                defaultTimeContent.innerHTML = `
                    ...
                    <div class="book-text-zinc-900 book-text-xs book-mb-4">
                        ${translate('time_from_label')} <span class="book-hour">5:00 ${translate('until_time')} 11:59</span>
                    </div>
                `;
                    ...
                    const airlineItems = (item.FlightGroup || []).flatMap(flight =>
                        (flight.RoutesInfo || []).map(value => ({
                            ...
                            Price: item.PriceInfo?.Total
                                ? priceWithCurrency(item.PriceInfo.Total, { as: 'number' })
                                : Infinity,
...
const renderCurrency = async (element) => {
    try {
        ...
        return `<span class="${marginClass} book-currency">${unit}</span>`;
    } catch (error) {
...
const renderFlightGroupPc = async (element) => {
    ...
    <div class="book-card__logo__class__provider book-text-center">
        ...
        <div class="book-mt-3">
            ...
            <div class="book-text-xs book-flex book-text-secondary-600 ${marginTopClass1}">
                <svg width="17" height="17" class="book-fill-secondary-500">
                    <use href="/booking/images/sprite-booking-icons.svg#seat-flight-icon"></use>
    ...
    <div class="book-card__route__map">
        <div class="book-flex book-justify-between book-mt-6">
            ...
            if (index === lastIndex) {
                output += `
        <div class="book-flex book-items-center ${marginTopClass2} book-gap-1 book-flex-wrap">
...
const renderFlightGroupPcB2B = async (element) => {
    try {
        ...
        // کلاس‌های راست‌به‌چپ/چپ‌به‌راست
        const marginLeftClass = isRTL ? "book-ml-1" : "book-mr-1";
        const marginRightClass = isRTL ? "book-mr-1" : "book-ml-1";
        const positionRelativeClass = isRTL ? "book-relative book-top-[3px]" : "";
        const minWidthClass = isRTL ? "book-min-w-24" : "book-min-w-28";

        const marginTopClass2 =
            element.FlightGroup.length === 1 &&
                element.PriceInfo?.PassengerFare?.[0]?.Commission !== "0"
                ? "book-mt-18"
                : "book-mt-3";
        const marginTopClass1 =
            element.FlightGroup.length === 1 &&
                element.PriceInfo?.PassengerFare?.[0]?.Commission !== "0"
                ? "book-mt-20"
                : "book-mt-6";

        // جمع مدت‌ها برای "Total travel time"
        let totalMinutes = 0;

        const groups = Array.isArray(element.FlightGroup) ? element.FlightGroup : [];
        const lastGroupIdx = groups.length - 1;

        for (const [gIdx, item] of groups.entries()) {
            const classMap = {
                economy: translate("economy_class"),
                businessclass: translate("business_class"),
                firstclass: translate("first_class"),
            ...
            const FlightClass =
                classMap[(item.Class || "").toLowerCase()] || translate("economy_class");

            const routes = Array.isArray(item.RoutesInfo) ? item.RoutesInfo : [];
            const lastSegIdx = routes.length - 1;

            for (const [sIdx, seg] of routes.entries()) {
                const isLastSection = gIdx === lastGroupIdx && sIdx === lastSegIdx;

                // داده‌های سگمنت
                const depTime = seg.DepartureTime || item.DepartureTime;
                const arrTime = seg.ArrivalTime || item.ArrivalTime;
                const depIata = seg.OriginAirport || item.Origin;
                const arrIata = seg.DestinationAirport || item.Destination;
                const segDuration = seg.Duration || item.Duration;
                const segAirline = seg.AirlineCode || item.RoutesInfo?.[0]?.AirlineCode;
                const segFlightNo = seg.FlightNumber || item.RoutesInfo?.[0]?.FlightNumber;
                const segClassCode = seg.ClassCode || item.RoutesInfo?.[0]?.ClassCode || "";

                // برای total time: جمع دقیقه‌های این سگمنت
                const dm = String(segDuration || "").match(/(\d+)\s*h\s*(\d+)\s*m/i);
                if (dm) totalMinutes += (+dm[1]) * 60 + (+dm[2] || 0);

                // بگج مطابق همین سگمنت (در صورت نبود، اولی)
                const segBaggage =
                    (Array.isArray(element.Baggages) &&
                        element.Baggages.find(
                            (b) =>
                                (b.SegmentId && seg.SegmentId && Number(b.SegmentId) === Number(seg.SegmentId)) ||
                                (b.Origin === depIata && b.Destination === arrIata)
                        )) ||
                    (Array.isArray(element.Baggages) ? element.Baggages[0] : null);

                // سکشن (یک Route)
                output += `
  <div class="book-flex book-justify-between book-px-4 book-py-3 ${!isLastSection ? 'book-border-b book-border-dashed book-border-zinc-200' : ''}">
    <!-- لوگو/ایرلاین/کلاس/صندلی -->
    <div class="book-card__logo__class__provider book-text-center">
      <div class="book-bg-zinc-50 book-border book-border-zinc-100 book-rounded">
        ${await renderAirlineLogo(segAirline, '2-13', '85', '34', item)}
      </div>
      <div class="book-mt-3">
        <div class="book-text-xs book-text-zinc-800 book-font-bold">
          ${await renderAirlineName(segAirline)}
        </div>
        <div class="book-text-xs book-text-zinc-500 book-my-3">
          ${translate("flight_number")}
          <span class="${marginRightClass}">${segFlightNo || ""}</span>
        </div>
        <div class="book-text-xs book-text-zinc-500">${FlightClass}</div>
        ${segClassCode ? `<div class="book-text-xs book-text-zinc-500">${segClassCode}</div>` : ``}
        <div class="book-text-xs book-flex book-text-secondary-600 ${marginTopClass1}">
          <svg width="17" height="17" class="book-fill-secondary-500">
            <use href="/booking/images/sprite-booking-icons.svg#seat-flight-icon"></use>
          </svg>
          <span class="${positionRelativeClass}">
            ${translate("remaining_seats")} :
            <span class="book-mx-1">${item.AvailableSeats ?? "-"}</span>${translate("seats")}
          </span>
        </div>
      </div>
    </div>
    ...
    <div class="book-card__route__map">
      <div class="book-flex book-justify-between book-mt-6">
        <div class="book-text-center">
          <div class="book-text-lg book-font-bold book-text-zinc-900">${depTime ?? "-"}</div>
          <div class="book-text-xs book-text-zinc-900 book-mt-1 book-mb-2">${await renderCity(depIata)}</div>
          <div class="book-text-xs book-text-zinc-400">${depIata ?? ""}</div>
        </div>
        ...
        <div class="book-relative book-mx-4 book-mt-4 book-text-center">
          <div class="book-w-full">
            <div class="book-flight__progress__line book-min-w-[29rem] book-relative">
              ...
              ${await renderNumberOfStops({ NumberOfStops: 0 }, 'pc')}
              <svg width="17" height="18" class="book-absolute book--left-2 book--bottom-0.5">
                <use href="/booking/images/sprite-booking-icons.svg#pin-icon"></use>
              </svg>
            </div>
          ...
          <!-- پِل زیر خط: اگر آخرین سکشن = Total travel time، در غیر اینصورت Connection time -->
          ${(() => {
                        if (isLastSection) {
                            const h = Math.floor(totalMinutes / 60);
                            const m = totalMinutes % 60;
                            return `
                <div class="book-inline-flex book-items-center book-justify-center book-gap-2 book-px-3 book-py-1 book-rounded-full book-bg-zinc-100 book-text-xs book-text-zinc-600 book-font-medium book-mt-2">
                  ${h}H ${String(m).padStart(2, '0')}M
                  <span>${translate?.("total_travel_time") || "Total travel time"}</span>
                </div>`;
                        } else {
                            // Connection pill (اگر مقدار داشت)
                            const conn = seg.ConnectionTime;
                            if (conn && conn !== "0" && conn !== "00:00") {
                                return `
                  <div class="book-inline-flex book-items-center book-justify-center book-gap-2 book-px-3 book-py-1 book-rounded-full book-bg-zinc-100 book-text-xs book-text-zinc-600 book-font-medium book-mt-2">
                    ${conn}
                    <span>${translate?.("connection_time") || "Connection time"}</span>
                  </div>`;
                            }
                            return ``;
                        }
          ...
          <!-- مدت همین سگمنت (بالای پِل مثل نمونه) -->
          <div class="book-text-xs book-text-zinc-500 book-mt-2 book-flex book-gap-1 book-justify-center">
            <svg width="17" height="17"><use href="/booking/images/sprite-booking-icons.svg#hour-icon"></use></svg>
            <span class="${positionRelativeClass}">${renderFormatterDuration(segDuration)}</span>
          </div>
        </div>
  
        <div class="book-text-center">
          <div class="book-text-lg book-font-bold book-text-zinc-900">${arrTime ?? "-"}</div>
          <div class="book-text-xs book-text-zinc-900 book-mt-1 book-mb-2">${await renderCity(arrIata)}</div>
          <div class="book-text-xs book-text-zinc-400">${arrIata ?? ""}</div>
  
          <!-- وزن بَگ کنار مقصد -->
          <div class="book-mt-2 book-text-xs book-text-zinc-500 book-inline-flex book-items-center book-gap-1">
            <svg width="14" height="14" class="book-fill-zinc-500">
              <use href="/booking/images/sprite-booking-icons.svg#bag-icon"></use>
            </svg>
            ${await renderBaggages(segBaggage, 'style')}
          </div>
        </div>
      </div>
  
      ${isLastSection ? `
      <!-- ردیف امکانات فقط یک‌بار، بعد از آخرین سکشن -->
      <div class="book-flex book-items-center ${marginTopClass2} book-gap-1 book-flex-wrap">
        <div class="book-bg-zinc-100 book-text-emerald-500 book-flex book-gap-1 book-rounded-full book-text-xs ${minWidthClass} book-py-2 book-items-center book-justify-center">
          <svg width="17" height="17" class="book-fill-emerald-500"><use href="/booking/images/sprite-booking-icons.svg#wheel-bag-cart-icon"></use></svg>
          ${await renderBaggages(segBaggage, 'style')}
        </div>
        <div class="book-bg-zinc-100 book-text-zinc-500 book-flex book-gap-1 book-rounded-full book-text-xs ${minWidthClass} book-py-2 book-items-center book-justify-center">
          <svg width="17" height="17"><use href="/booking/images/sprite-booking-icons.svg#class-icon"></use></svg>
          <span><span class="${marginLeftClass}">${FlightClass}</span>${segClassCode ? `<span class="${marginLeftClass}">${segClassCode}</span>` : ""}</span>
        </div>
        <div class="book-bg-zinc-100 book-text-zinc-500 book-flex book-gap-1 book-rounded-full book-text-xs ${minWidthClass} book-py-2 book-items-center book-justify-center">
          <svg width="17" height="17"><use href="/booking/images/sprite-booking-icons.svg#ticket-icon"></use></svg>
          <span>${await renderIsSystemFlight(item.isSystemFlight)}</span>
        </div>
        ${await renderEnablePoint(element, 'pc')}
        ${await renderFareFamily(item.FareFamily, 'pc')}
      ...
      ` : ``}
        ...
        console.error("renderFlightGroupPcB2B: " + error.message);
        return "";
    }
};
        ...
        if (element === "true" || element === true) {
            if (type == 'mob') {
                ...
                <div class="book-text-zinc-800 book-rounded-full book-px-2 book-bg-zinc-100 book-flex book-gap-1 book-text-xs book-items-center">
                  <span>${translate("special_rate")}</span>
                </div>`;
            } else {
                return `
               <div class="book-bg-zinc-100 book-text-zinc-500 book-flex book-gap-1 book-rounded-full book-text-xs ${minWidthClass} book-py-2 book-items-center book-justify-center">
...
const renderConnectionTime = async (element) => {
    try {
        if (element > 0) {
            ...
            const marginRightClass = isRTL ? 'book-mr-1 book-relative book-top-[2px]' : 'book-ml-1';

            return `<div class="book-text-xs book-text-zinc-600 ${isMobile ? 'book-my-2' : 'book-mt-2'} book-flex book-items-center">
              <svg width="15" height="15" class="${isMobile ? '' : 'book-mx-auto'}">
                  <use href="/booking/images/sprite-booking-icons.svg#time-circle-icon"></use>
              </svg>
              <span class="${marginRightClass}">${hours}:${minutes}</span>
          </div>`;
        }
        return "";
    } catch (error) {
...
<div class="book-route__info">
    ...
    <div class="book-flex book-mb-4">
        <div class="book-flex">
            ...
            <div class="book-flex book-flex-col ${isRTL ? 'book-border-l book-ml-7 ' : 'book-border-r book-mr-7 '} book-items-center book-justify-between book-border-zinc-300 book-px-7">
                <div>
        ...
        if (element?.ConnectionTime > 0) {
            ...
            return `<div class="book-my-10 book-flex book-text-zinc-800 book-text-sm book-justify-between book-bg-zinc-100 book-rounded-xl book-p-3">
              <div class="book-flex book-gap-1">
                  <svg class="book-relative book-top-[3px]" width="15" height="16">
            ...
            if (item.Count > 0) {
                ...
                output += `<ul class="${ulClass}">
                   <li class="book-flex book-justify-between book-py-3 book-px-2 book-bg-zinc-100 book-rounded-lg book-mb-2">
                      ...
                      <span><span class="book-ltr book-inline-block">${await priceWithCurrency(item.Unit)}</span>${await renderCurrency(element.PriceInfo.Currency)}</span>
                  </li>
                  <li class="book-flex book-justify-between book-py-3 book-px-2 book-bg-zinc-100 book-rounded-lg book-mb-2">
                      <span>${translate("base_fare")}</span>
                      <span><span class="book-ltr book-inline-block">${await priceWithCurrency(item.BaseFare)}</span>${await renderCurrency(element.PriceInfo.Currency)}</span>
                  </li>
                  <li class="book-flex book-justify-between book-py-3 book-px-2 book-bg-zinc-100 book-rounded-lg book-mb-2">
                      <span>${translate("taxes_and_fees")}</span>
                      <span><span class="book-ltr book-inline-block">${await priceWithCurrency(item.Tax)}</span>${await renderCurrency(element.PriceInfo.Currency)}</span>
                  </li>
                    <li class="book-flex book-justify-between book-py-3 book-px-2 book-bg-zinc-100 book-rounded-lg book-mb-2">
                      <span>${translate("taxes_and_services")}</span>
                      <span><span class="book-ltr book-inline-block">${await priceWithCurrency(item.providerFare)}</span>${await renderCurrency(element.PriceInfo.Currency)}</span>
                  </li>`;
                if (item.Commission && item.Commission !== '0') {
                    output += `<li class="book-flex book-justify-between book-py-3 book-px-2 book-bg-zinc-100 book-rounded-lg book-mb-2">
                      ...
                      <span><span class="book-ltr book-inline-block">${await priceWithCurrency(item.Total)}</span>${await renderCurrency(element.PriceInfo.Currency)}</span>
                        ...
                        <span><span class="book-ltr book-inline-block">${await priceWithCurrency(item.Commission)}</span>${await renderCurrency(element.PriceInfo.Currency)}</span>
                    </li><li class="book-flex book-justify-between book-py-3 book-px-2 book-bg-zinc-100 book-rounded-lg book-mb-2">
                        ...
                        <span><span class="book-ltr book-inline-block">${await priceWithCurrency(item.TotalCommission)}</span>${await renderCurrency(element.PriceInfo.Currency)}</span>
                    </li></ul>`;
                } else {
                    output += `<li class="book-flex book-justify-between book-py-3 book-px-2 book-bg-zinc-100 book-rounded-lg book-mb-2">
                      <span>${translate("total_price")} ${passengerType}<span class="book-mx-1">(${item.Count}<span class="book-mx-1">${translate("people")}</span>)</span></span>
...
const renderPerPrice = async (element) => {
    try {
        ...
        if (commission === 0) {
            ...
            ${await priceWithCurrency(Math.floor(totalCommission / count))}
            ...
            output = `
                <div class="book-mb-2">
                   <div class="book-text-xs book-text-zinc-500 book-mb-2">${translate("commission_per_passenger")}:</div>
                    ...
                    ${await priceWithCurrency(perCommission)}
                    </span>${await renderCurrency(element.Currency)}</h4>
                </div>
                <div>
...
const renderBaseFare = async (element, unit, id) => {
    try {
        ...
        if (differencePrice === 0) {
            ...
            return `<div class="book-baseFare__btn book-active__baseFare__btn book-p-2 book-text-sm book-text-center book-rounded-3xl book-mt-6 book-rtl">${translate("selected")}</div>`;
        }
        ...
        return `<div class="book-baseFare__btn book-bg-primary-50 book-p-2 book-text-sm book-text-center book-text-primary-300 book-rounded-3xl book-mt-6 book-rtl">
          <span class="book-addition__price__container book-font-bold">
...
const selectAmenities = async (element, idToFind) => {
    try {
        ...
        submitButton.textContent = additionPrice == 0 ? translate("increase_fare_and_continue") : translate("continue_with_current_fare");
