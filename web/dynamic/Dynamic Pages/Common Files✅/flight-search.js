...
const renderProvider = (element) => {
    try {
        ...
        if (providerData) {
            ...
            return `<div class="book-bg-zinc-100 book-text-zinc-500 book-flex book-gap-1 book-rounded-full book-text-xs book-min-w-24 book-py-2 book-items-center book-justify-center ${marginLeftClass}">
                ...
                if (item.Commission && item.Commission !== '0') {
                    output += `<li class="book-flex book-justify-between book-py-3 book-px-2 book-bg-zinc-100 book-rounded-lg book-mb-2">
                      ...
                      <span><span class="book-ltr book-inline-block">${new Intl.NumberFormat().format(item.Total)}</span>${await renderCurrency(element.PriceInfo.Currency)}</span>
                        ...
                        <span>${translate("amount_after_commission")} ${passengerType}</span>
...
const renderPerPrice = async (element) => {
    try {
        ...
        if (commission === 0) {
            ...
            output = `
                <div class="book-mb-2">
                   <div class="book-text-xs book-text-zinc-500 book-mb-2">${translate("commission")}:</div>
