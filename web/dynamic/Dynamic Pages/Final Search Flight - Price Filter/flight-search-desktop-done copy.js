...
const manipulation = async (args) => {
  ...
  if (!priceSlider) {
    console.error("manipulation: Price slider not found");
    return;
  }
  if (!args.source.rows || !Array.isArray(args.source.rows) || args.source.rows.length === 0) {
    console.error("manipulation: Invalid source rows for cms.price");
    return;
  }

  // --- تفاوت اینجاست: دسکتاپ همون قبلی، موبایل فقط تاچ ---
  if (!isMobile) {
    // ======== کد فعلیِ رویدادهای ماوس شما بدون تغییر ========
    let isDragging = false;
    const onMouseMove = (e) => {
    ...
    // ======== موبایل: فقط تاچ ========
    enablePriceTouchDragForPriceSlider();
  }
}
...
function handlePriceDrag(args) {
    try {
        ...
        thumbElement.addEventListener("mouseup", currentMouseUpHandler, { once: true }); 
...
// --- فقط یک بار: هندل تاچ برای اسلایدر قیمت روی موبایل ---
function enablePriceTouchDragForPriceSlider() {
  try {
    if (!isMobile) return;                 // فقط موبایل
    if (!priceSlider || !priceThumbMin || !priceThumbMax) return;
    if (priceSliderInitialized) return;    // از دوبار بایند جلوگیری کن
    ...
    const onTouchMoveMin = (e) => {
      ...
      if (percent <= maxPercent) {
        minPercent = percent;
        updatePriceSlider();               // تابع موجود در پروژه شما
      }
      e.preventDefault();
    };
    ...
    const onTouchEnd = () => {
      // ست‌کردن فلگ‌ها و اعمال فیلتر مثل دسکتاپ
      userHasChangedPriceRange = true;
      lastUserMinPrice = priceRange[0];
      lastUserMaxPrice = priceRange[1];
      ...
      clearTimeout(window.priceFilterTimeout);
      window.priceFilterTimeout = setTimeout(() => {
        triggerPriceFilter();              // تابع موجود در پروژه شما
      }, DEBOUNCE_DELAY);

      // پاک‌کردن لیسنرهای موقت
      document.removeEventListener('touchmove', onTouchMoveMin);
      document.removeEventListener('touchmove', onTouchMoveMax);
      document.removeEventListener('touchend', onTouchEnd);
    };

    // شروعِ درگ برای هر هندل
    priceThumbMin.addEventListener('touchstart', (e) => {
      document.addEventListener('touchmove', onTouchMoveMin, { passive: false });
      document.addEventListener('touchend', onTouchEnd, { passive: false });
    }, { passive: false });

    priceThumbMax.addEventListener('touchstart', (e) => {
      document.addEventListener('touchmove', onTouchMoveMax, { passive: false });
      document.addEventListener('touchend', onTouchEnd, { passive: false });
    }, { passive: false });

  } catch (err) {
    console.error('enablePriceTouchDragForPriceSlider:', err.message);
  }
}
