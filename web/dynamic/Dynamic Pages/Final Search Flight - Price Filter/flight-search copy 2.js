...
let durationUserHasChanged = false;
let durationLastUserMinHour = 0;
let durationLastUserMaxHour = 0;
        ...
        if (TripGroup.length > 0) {
            ...
            if (schemaId === 291) {
                ...
                TripGroup.forEach((trip, index) => {
                    ...
                    tripNameDiv.textContent = `${translate("route")} ${tripNames[index]}`;
...
const manipulation = async (args) => {
    ...
    if (args.source.id === 'cms.page') {
        ...
        if (!args.source.rows || !Array.isArray(args.source.rows) || args.source.rows.length === 0) {
            console.error("manipulation: Invalid source rows for cms.hour");
            return;
        }
    ...
    } else if (args.source.id === "cms.hour.update") {
        InUpdateFiltering = false;
        InUpdatePaging = true;
        selectedFlightId = null;
        mustUpdate = true;
...
const renderCurrency = async (element) => {
    try {
        ...
        return `<span class="${marginClass} book-currency">${mergedCurrency[element] || ""}</span>`;
    } catch (error) {
    ...
    try {
        ...
        <div class="book-flex book-items-center book-justify-between book-text-xs">
          ...
          <div>
            ...
            ${await renderBaggages(Array.isArray(element.Baggages) && element.Baggages.length > 0 ? element.Baggages[0] : null)}
            </div>
...
const renderBaseFare = async (element, unit, id) => {
    try {
        ...
        return `<div class="book-baseFare__btn book-bg-primary-50 book-p-2 book-text-sm book-text-center book-text-primary-300 book-rounded-3xl book-mt-6 book-rtl">
          <span class="book-addition__price__container book-font-bold">${new Intl.NumberFormat().format(differencePrice)}</span>${await renderCurrency(unit)}
...
function setupDesktopPriceSlider(thumbType) {
    try {
        ...
        const onMouseMove = (e) => {
            ...
            if (thumbType === "min" && percent <= maxPercent) {
                minPercent = percent;
            } else if (thumbType === "max" && percent >= minPercent) {
                maxPercent = percent;
            }
            updatePriceSlider();
        };
        ...
        console.log(`Desktop price slider ${thumbType} drag started`);
...
function setupMobilePriceSlider(thumbType) {
    try {
        ...
        const updateSliderFromTouch = (clientX) => {
            ...
            if (currentThumb === "min" && percent <= maxPercent) {
                minPercent = percent;
            } else if (currentThumb === "max" && percent >= minPercent) {
                maxPercent = percent;
            }
            updatePriceSlider();
        };
        ...
        console.log(`Mobile price slider ${thumbType} drag started`);
...
const updatePriceSlider = () => {
    try {
        ...
        console.log("Price slider updated:", { minPercent, maxPercent, priceRange });
...
function cleanupDurationSliderEvents() {
    try {
        if (window.durationMouseMoveHandler) {
            document.removeEventListener("mousemove", window.durationMouseMoveHandler);
            delete window.durationMouseMoveHandler;
        }
        if (window.durationMouseUpHandler) {
            document.removeEventListener("mouseup", window.durationMouseUpHandler);
            delete window.durationMouseUpHandler;
        }

        const hourContainer = document.querySelector(".book-hour__content .book-filter__hour__container");
        if (hourContainer) {
            const thumbMin = hourContainer.querySelector('.book-thumb__min');
            const thumbMax = hourContainer.querySelector('.book-thumb__max');
            
            if (thumbMin && thumbMin._touchStartHandler) {
                thumbMin.removeEventListener('touchstart', thumbMin._touchStartHandler);
                delete thumbMin._touchStartHandler;
            }
            if (thumbMax && thumbMax._touchStartHandler) {
                thumbMax.removeEventListener('touchstart', thumbMax._touchStartHandler);
                delete thumbMax._touchStartHandler;
            }
        }

        if (window.durationTouchMoveHandler) {
            document.removeEventListener('touchmove', window.durationTouchMoveHandler);
            delete window.durationTouchMoveHandler;
        }
        if (window.durationTouchEndHandler) {
            document.removeEventListener('touchend', window.durationTouchEndHandler);
            document.removeEventListener('touchcancel', window.durationTouchEndHandler);
            delete window.durationTouchEndHandler;
        }
    } catch (error) {
...
function setupDesktopDurationSlider(thumbType) {
    try {
        ...
        window.durationMouseMoveHandler = onMouseMove;
        window.durationMouseUpHandler = onMouseUp;

        isDragging = true;
        document.addEventListener("mousemove", onMouseMove);
        document.addEventListener("mouseup", onMouseUp);

        console.log(`Desktop duration slider ${thumbType} drag started`);
...
function setupMobileDurationSlider(thumbType) {
    try {
        ...
        window.durationTouchMoveHandler = onTouchMove;
        window.durationTouchEndHandler = onTouchEnd;
        ...
        console.log(`Mobile duration slider ${thumbType} drag started`);
...
function applyDurationFilter() {
    try {
        ...
        console.log("Applying duration filter:", hourRange);
...
// =============================================================================
// UPDATE SLIDER UI FUNCTIONS
// =============================================================================

const updateDurationSliderUI = () => {
    try {
        const hourContainer = document.querySelector(".book-hour__content .book-filter__hour__container");
        const totalRange = maxHour - minHour;
        const durationMinValue = Math.round((minPercent / 100) * totalRange + minHour);
        const durationMaxValue = Math.round((maxPercent / 100) * totalRange + minHour);

        hourRange = [durationMinValue, durationMaxValue];

        if (hourMinValueLabel) {
            hourMinValueLabel.textContent = convertToTime(durationMinValue);
        }
        if (hourMaxValueLabel) {
            hourMaxValueLabel.textContent = convertToTime(durationMaxValue);
        }

        const thumbMin = hourContainer.querySelector('.book-thumb__min');
        const thumbMax = hourContainer.querySelector('.book-thumb__max');
        const track = hourContainer.querySelector('.book-slider__track');
        ...
        console.log("Duration slider updated:", { minPercent, maxPercent, hourRange });
...
function initializeDurationSliders() {
    try {
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', initializeDurationSliders);
            return;
        }

        // Setup main duration slider  
        const hourContainer = document.querySelector(".book-hour__content .book-filter__hour__container");
        if (hourContainer && !durationSliderInitialized) {
            const thumbMin = hourContainer.querySelector('.book-thumb__min');
            const thumbMax = hourContainer.querySelector('.book-thumb__max');
            ...
            if (thumbMin) {
                ...
                if (isMobile) {
                    thumbMin._touchStartHandler = (e) => {
                        e.preventDefault();
                        cleanupDurationSliderEvents();
                        setupMobileDurationSlider('min');
                    };
                    thumbMin.addEventListener('touchstart', thumbMin._touchStartHandler, { passive: false });
                }
            }
            ...
            if (thumbMax) {
                ...
                if (isMobile) {
                    thumbMax._touchStartHandler = (e) => {
                        e.preventDefault();
                        cleanupDurationSliderEvents();
                        setupMobileDurationSlider('max');
                    };
                    thumbMax.addEventListener('touchstart', thumbMax._touchStartHandler, { passive: false });
                }
            }
        ...
        console.log("Duration slider initialized successfully");
...
const convertToTime = (minutes) => {
    try {
        ...
        if (m > 0) result += (h > 0 ? ' ' : '') + `${m} ${translate("minute")}`;
