...
const updatePassengerUI = () => {
    try {
        ...
        if (adultsCount > 0) passengerParts.push(`${adultsCount} ${translate("بزرگسال")}`);
        if (childrenCount > 0) passengerParts.push(`${childrenCount} ${translate("کودک")}`);
        if (infantsCount > 0) passengerParts.push(`${infantsCount} ${translate("نوزاد")}`);
        ...
        if (moduletype === 'bus') {
            ...
            routeBlocks.forEach((route, index) => {
                ...
                if (returnDateContainer && !returnDateContainer.classList.contains("disabled__date__container") && !returnDate) {
                    hasError = true;
                    errorBox.innerHTML += `${translate("مسیر")} ${index + 1}: ${translate("return_date_required")}.<br>`;
                }
