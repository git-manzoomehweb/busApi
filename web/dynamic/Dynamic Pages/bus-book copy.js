...
const onProcessedUserCredit = async (args) => {
  try {
    ...
    if (response.status === 200) {
      ...
      if (responseJson) {
        ...
        if (userCredit > firstPay) {
          ...
          document
            .querySelector(".book-invoice__container")
            ?.insertAdjacentHTML(
              "beforeend",
              `
                                <div class="book-invoice__content book-cursor-pointer book-p-2" data-run="0" onclick="submitInvoice(this,'credit__Invoice')">
                                    <ul class="book-flex book-justify-between">
                                        ...
                                        <li><p>اعتبار باقی مانده :</p>
                                            ...
                                            <span class="book-text-xs book-mx-1">${
                                              document.querySelector(
                                                ".book-unit__content span"
                                              ).textContent
                                            }</span></li>
                                        <li><span class="book-totalcom__cost">${
                                          document.querySelector(
                                            ".book-firstpay__cost"
                                          ).textContent
                                        }</span>
    ...
    console.error(
      `onProcessedUserCredit: ${err.message}, Line: ${
        err.lineNumber || "unknown"
      }`
    );
  }
  ...
  try {
    ...
    if (
      !event.target.closest(
        ".book-has__drop__item input, .book-has__drop__item .book-drop__item__content"
      )
    ) {
      document.querySelectorAll(".book-drop__item__content").forEach((e) => {
        e.classList.remove("book-drop__item__content-toggle");
    ...
    if (!event.target.closest(".book-select__item__container")) {
      ...
      if (previousPassengersContainer) {
        previousPassengersContainer
          .querySelectorAll(".book-select__item__content")
          .forEach((e) => {
            e.classList.add("book-hidden");
          });
      }
    ...
    console.error(
      `handleClickOutside: ${err.message}, Line: ${err.lineNumber || "unknown"}`
    );
  }
};
...
const generateDays = (dropdownId) => {
  try {
    ...
    for (let i = 1; i <= 31; i++) {
      ...
      li.setAttribute(
        "onclick",
        "selectDropItem(this, 'book-date__item__content')"
      );
...
const generateYears = (dropdownId, isGregorian = false, isFuture = false) => {
  try {
    ...
    if (isGregorian) {
      ...
      const startYear = isFuture
        ? currentGregorianYear
        : currentGregorianYear - 100;
      const endYear = isFuture
        ? currentGregorianYear + 20
        : currentGregorianYear;
...
//       stack.className =
//         "book-fixed book-right-6 book-top-6 book-z-50 book-space-y-3 book-pointer-events-none";
//       document.body.appendChild(stack);
//     } else {
//       // اگر قبلا پایین بوده، به بالا منتقلش کن
//       stack.classList.remove("book-bottom-6");
//       stack.classList.add("book-top-6");
//     }
...
//     toast.className =
//       "book-pointer-events-auto book-max-w-md book-font-bold book-border" +
//       "book-bg-red-300 book-text-red-900 book-border-red-600 book-rounded-lg " +
//       "book-shadow-lg book-text-sm book-leading-6 " +
//       "book-px-4 book-py-3 " +
//       "book-opacity-0 book--translate-y-2 " + // از بالا بیاد داخل
