...
// Global variables to maintain data state
let globalBusProposals = []; // جهانی کردن برای نگهداری همه داده‌ها
let globalListData = null; // برای نگهداری ساختار اصلی
...
// متغیرهای مدیریت event listener
let currentDragType = null; // ⭐ اضافه شده برای تشخیص نوع drag
...
// 🔥 تابع اصلاح شده busManipulation - جایگزین کردن کامل
const busManipulation = async (args) => {
    try {
        ...
        if (args.source.id === 'cms.page') {
            ...
            InUpdatePaging = true;
            selectedBusId = null;
            console.log("carrier:", args.source);
            if (!args.source.rows || !Array.isArray(args.source.rows) || args.source.rows.length === 0) {
