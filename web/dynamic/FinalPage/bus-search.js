  ...
  try {
    ...
    if (args.source.id === "cms.page") {
      ...
      const onMouseMove = (e) => {
        ...
        const x = Math.min(
          Math.max(e.clientX - priceSlider.getBoundingClientRect().left, 0),
          priceSlider.getBoundingClientRect().width
        );
      ...
      document.addEventListener("mousedown", function (e) {
        ...
        if (
          target.classList &&
          [...target.classList].some((cls) => cls.startsWith("book-thumb__"))
        ) {
          isDragging = true;
...
function initializeBusCards(idToFind, type) {
  try {
      ...
      for (let j = 0; j < item.rule.length; j++) {
        ...
        if (rule.title === "cancel rule") {
          ...
          for (let condition of conditions) {
            ...
            ruleDiv.className =
              "book-w-[90%] book-h-10 book-flex book-justify-center book-items-center book-text-sm book-text-zinc-700 book-p-1 book-relative book-bg-transparent book-border book-border-zinc-500 book-rounded-lg";
            ruleDiv.innerHTML = `
