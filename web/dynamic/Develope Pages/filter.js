// تابع فرمت‌دهی داده‌های اتوبوس مطابق با خروجی JSON جدید
function formatBusData(dataResponse) {
    listData = dataResponse;
    console.log(listData);
    console.table(listData);
    
    // دریافت داده اصلی از ردیف اول
    const data = dataResponse.source._rows[0];
    const enriched = [];
    
    // استخراج دیکشنری‌ها
    const locationDict = data.dictionaries?.location || {};
    const carrierDict = data.dictionaries?.carriers || {};
    const currencyDict = data.dictionaries?.currency || {};
    
    // پردازش هر پیشنهاد اتوبوس
    (data.busProposals || []).forEach((proposal, index) => {
        const busGroup = proposal.busGroup?.[0] || {};
        const routeInfo = busGroup.routesInfo?.[0] || {};
        
        // استخراج اطلاعات قیمت
        const price = proposal.priceInfo?.total || 0;
        const totalCommission = proposal.priceInfo?.totalCommission || 0;
        const currencyCode = proposal.priceInfo?.currency || '';
        
        // استخراج شناسه‌های مبدا و مقصد
        ...
        enriched.push({
            ...
            // اطلاعات قیمت
            baseFare: proposal.priceInfo?.baseFare || 0,
            tax: proposal.priceInfo?.tax || 0,
            providerFare: proposal.priceInfo?.providerFare || 0,
            price: price,
    ...
    // تنظیم منبع داده در BasisCore
    $bc.setSource('busData.formatted', enriched);
    return enriched;
}

// تابع اصلاح شده busManipulation برای اتوبوس
...
function setupBookCardButtons() {
    ...
    bookButtons.forEach(button => {
        button.addEventListener('click', function() {
            ...
            if (busId) {
                $bc.setSource('cms.bus', [{ value: busId }]);
            }
        });
    });
}
...
function updatePriceSlider() {
    ...
    const minPrice = (maxPrice - minPrice) * (minPercent / 100) + minPrice;
    ...
    if (priceMinValueLabel) {
        priceMinValueLabel.textContent = new Intl.NumberFormat('fa-IR').format(Math.round(minPrice));
    }
